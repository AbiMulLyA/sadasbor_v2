...
class DashboardPostsCubit extends Cubit<DashboardPostsState> {
  ...
  Future<void> getPosts({
    ...
    required PagingState<int, DashboardPostsEntity> currentState, // Ganti PagingController dengan PagingState
    required Function(PagingState<int, DashboardPostsEntity>) updateState, // Callback untuk update state
    bool retryData = true,
    String? keywords,
  }) async {
    emit(_Loading());
    ...
    return result.fold(
        ...
        if (isLastPage) {
          updateState(currentState.copyWith(
            pages: [...?currentState.pages, data], // Tambahkan page baru
            keys: [...?currentState.keys, pageKey], // Tambahkan key baru
            hasNextPage: false, // Tandai sebagai halaman terakhir
            isLoading: false,
            error: null,
          ));
        } else {
          updateState(currentState.copyWith(
            pages: [...?currentState.pages, data], // Tambahkan page baru
            keys: [...?currentState.keys, pageKey], // Tambahkan key baru
            hasNextPage: true, // Masih ada halaman selanjutnya
            isLoading: false,
            error: null,
          ));
        }
